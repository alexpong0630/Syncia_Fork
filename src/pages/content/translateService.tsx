...
class TranslateService {
  ...
  updateSettings = (settings: Settings, autoStart = true) => {
  ...
  start = () => {
    if (!this.settings.autoTranslation.enabled) return

    if (
      this.settings.autoTranslation.autoTranslateForDomain.indexOf(
  ...
  translationsThisMinute = () => {
    return new Promise((resolve) => {
      chrome.storage.local.get(
        ['translationsThisMinute', 'lastTranslationTime'],
        (result) => {
          ...
          if (lastTranslationTime) {
            ...
            if (
              lastTranslationDate.getFullYear() !== currentDate.getFullYear() ||
              ...
              chrome.storage.local.set(
                { translationsThisMinute: 0, lastTranslationTime: Date.now() },
                () => {
                  contentScriptLog(
                    'translationsThisMinute reset to 0 due to minute change',
                  )
                },
              )
            }
  ...
  addTranslationsThisMinute = async () => {
    const currentTranslations = (await this.translationsThisMinute()) as number
    chrome.storage.local.set(
      {
        translationsThisMinute: currentTranslations + 1,
        lastTranslationTime: Date.now(),
      },
      () => {
        contentScriptLog(
          `translationsThisMinute incremented to ${currentTranslations + 1}`,
        )
      },
    )
  }
  ...
  insertAfter = (newNode: HTMLElement, referenceNode: HTMLElement) => {
    ...
    if (parent) parent.insertBefore(newNode, referenceNode.nextSibling || null)
  }
  ...
  assignUUIDForDOMs = (domElements: HTMLCollection) => {
    for (var element of domElements) {
      ...
      if (
        !element.classList ||
        !this.tagsForTranslation.includes(tag) ||
        element.classList.contains('autotrans-translated') ||
      ...
      if (element.children.length > 0) {
        if (
          this.tagsForExtractText.includes(tag) &&
          element.textContent &&
  ...
  addTranslatedElement = (uuid: string, text: string) => {
    ...
    const domElement = domElements[0] as HTMLElement
  ...
  callOpenAIChat = async (jsonContent: string) => {
    const expandedQuery =
      'translate below text to ' +
      this.settings.autoTranslation.language +
      '. do not add markdown. keep the order. do not explain the result.:\n' +
      jsonContent
    const messages = [
      new HumanMessage({
  ...
  callChatCompletion = async (toBeTranslateDomElementsBatch: any) => {
    var data = toBeTranslateDomElementsBatch
      .map((e: any) => e.value)
      .join('|')
      .replace(/[\n\r\t]/g, '')
    const response: any = await this.callOpenAIChat(data)
    ...
    for (var k = 0; k < translateTexts.length; k++) {
      if (toBeTranslateDomElementsBatch[k]) {
        this.addTranslatedElement(
          toBeTranslateDomElementsBatch[k].key,
          translateTexts[k],
        )
      }
    }
  ...
  translate = async () => {
    ...
    var toBeTranslateDomElements_temp = JSON.parse(
      JSON.stringify(this.toBeTranslateDomElements),
    )
    console.log(
      'API call in this minute:',
      await this.translationsThisMinute(),
      'Batch Size:',
      this.batchSize,
    )
    for (
      var i = 0;
      i < toBeTranslateDomElements_temp.length;
      i += this.batchSize
    ) {
      var toBeTranslateDomElementsBatch = toBeTranslateDomElements_temp
      ...
      while (
        ((await this.translationsThisMinute()) as number) >=
        this.maxTranslationThisMinute
      ) {
    ...
    this.toBeTranslateDomElements.splice(
      0,
      toBeTranslateDomElements_temp.length,
    )
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (action === 'syncia_cancel_translate' || action === 'syncia_translate') {
    chrome.storage.sync.get(['SETTINGS'], async (result) => {
      const autoTranslationSetting = (result.SETTINGS as Settings)
        .autoTranslation
      const host = request.payload.pageDomain
      if (action === 'syncia_cancel_translate') {
        autoTranslationSetting.autoTranslateForDomain =
          autoTranslationSetting.autoTranslateForDomain.filter(
            (domain) => domain !== host,
          )
      ...
      chrome.storage.sync.set({
        SETTINGS: {
          ...result.SETTINGS,
          autoTranslation: autoTranslationSetting,
        },
      })
      updateTranslateService()
    })
